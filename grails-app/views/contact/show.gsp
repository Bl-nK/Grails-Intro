	...
	<body>
		...
		<div id="show-contact" class="content scaffold-show" role="main">
			...
			<ol class="property-list contact">
				...
				<g:if test="${contactInstance?.customer}">
				<li class="fieldcontain">
					<span id="customer-label" class="property-label"><g:message code="contact.customer.label" default="Customer" /></span>
					
						<span class="property-value" aria-labelledby="customer-label"><g:link controller="customer" action="show" id="${contactInstance?.customer?.id}">${contactInstance?.customer?.encodeAsHTML()}</g:link></span>
